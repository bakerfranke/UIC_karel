"""Shared test support for BigCarpetBot - one function per gradable test case.
...
CLASS_NAME = "BigCarpetBot"
SOLVING_METHOD = "carpetAllRooms"
ROBOT_VAR = "carpee"

# Required starting (street, avenue, direction, beepers) - 8 possible rooms
# (avenues 2-9 inclusive), up to 3 beepers each (height 1-3), worst case all complete.
START_STATE = (1, 1, East, 24)
...
WORLD_FILES = [f"bigcarpetbot_world{i}.kwld" for i in range(10)]
...
def test_structure(test_feedback):
    ...
    allowed for-range form, BigCarpetBot exists and defines carpetAllRooms(), no
    ...
    when carpetAllRooms() runs on a fresh robot."""
...
def test_solves_on_its_own(test_feedback):
    """Checks that calling carpetAllRooms() directly on a fresh robot - bypassing
...
def test_main_guard_purity(test_feedback):
    """Confirms __main__ does nothing except construct a single BigCarpetBot and
    call carpetAllRooms() (turnOff() is fine too) - no extra problem-solving
