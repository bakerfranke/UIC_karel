"""Shared test support for SorterBot - one function per gradable test case.
...
CLASS_NAME = "SorterBot"
SOLVING_METHOD = "sortBeepers"
ROBOT_VAR = "sorty"

# Used only to test sortBeepers() in isolation, bypassing main.py - SorterBot has no
# required starting position, so any legal position/direction works here. Beeper
# count must be 0, though: the whole problem is solvable without ever needing a
# starting stash, and a correct solution can only tell "beepers I just picked up
# off this row" apart from "beepers already in my bag" if it starts with none.
START_STATE = (1, 1, East, 0)

# No required ending position/direction for SorterBot - only that it turns off.
EXPECTED_END_STATE = None

MODEL_FILE = "model_solution.py"
WORLD_FILES = [f"sorter_world{i}.kwld" for i in range(10)]
...
def test_structure(test_feedback):
    ...
    allowed for-range form, SorterBot exists and defines sortBeepers(), no method
    calls the global robot instance by name instead of self, and at least
    MIN_OTHER_METHODS other methods that are both DEFINED and actually CALLED
    when sortBeepers() runs on a fresh robot."""
...
def test_solves_on_its_own(test_feedback):
    """Checks that calling sortBeepers() directly on a fresh robot - bypassing
...
def test_ending_state(test_feedback):
    """Checks that sorty calls turnOff() by the end of main.py's run."""
    if not test.checkNoCodeOutsideMainGuard("main.py"):
        return False
    if not test.checkNoLoops("main.py"):
        return False
    ...
    if EXPECTED_END_STATE is not None:
...
def test_main_guard_purity(test_feedback):
    """Confirms __main__ does nothing except construct a single SorterBot and
    call sortBeepers() (turnOff() is fine too) - no extra problem-solving work
    hiding directly in main, and no extra robots constructed there either."""
    if not test.checkNoCodeOutsideMainGuard("main.py"):
        return False
    if not test.checkNoLoops("main.py"):
        return False
...
def test_beeper_conservation(test_feedback):
    """SorterBot only: checks that running main.py never changes the TOTAL
    number of beepers anywhere in the world, for every world file - sorting
    should only rearrange existing beepers, never add or remove any."""
    if not test.checkNoCodeOutsideMainGuard("main.py"):
        return False
    if not test.checkNoLoops("main.py"):
        return False

    passed = test.checkBeeperConservation("main.py", WORLD_FILES, CLASS_NAME, SOLVING_METHOD, START_STATE)
    if not passed:
        return False

    test_feedback.write("Beeper count is conserved on every world file - no beepers created or destroyed!")
    return True
...
if __name__ == "__main__":
    ...
    for fn in [test_structure, test_solves_via_main, test_solves_on_its_own,
               test_ending_state, test_main_guard_purity, test_beeper_conservation]:
